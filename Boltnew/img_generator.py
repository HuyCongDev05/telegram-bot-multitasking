...
def _html_to_png(html_content: str, width: int = 1200, height: int = None) -> bytes:
    ...
    try:
        ...
        try:
            # Thiết lập nội dung HTML trực tiếp, dùng domcontentloaded thay vì networkidle (nhanh hơn)
            page.set_content(html_content, wait_until='domcontentloaded')

            # Đợi tải ảnh (nếu có ảnh từ bên ngoài)
...
if __name__ == '__main__':
    ...
    for asset in assets:
        with open(asset["file_name"], "wb") as f:
            f.write(asset["data"])
        print(f"Đã tạo {asset['file_name']} ({len(asset['data'])} bytes)")
