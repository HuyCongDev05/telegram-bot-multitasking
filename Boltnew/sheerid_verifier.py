...
class SheerIDVerifier:
    ...
    def create_verification(self) -> str:
        ...
        if status != 200 or not isinstance(data, dict) or not data.get("verificationId"):
            raise Exception(f"Tạo xác thực thất bại (mã trạng thái {status}): {data}")

        self.verification_id = data["verificationId"]
        logger.info(f"✅ Lấy được verificationId: {self.verification_id}")
    ...
    def verify(
        ...
        try:
            ...
            if not self.verification_id:
                logger.info("Đang yêu cầu verificationId mới...")
            ...
            logger.info(f"✅ Bước 2 hoàn thành: {getattr(step2_data, 'get', lambda k, d=None: d)('currentStep')}")
            current_step = (
            ...
            # Yêu cầu tải lên và tải tài liệu lên
            logger.info("Bước 4/5: Yêu cầu URL tải lên...")
            step4_body = {
                "files": [
                    {
            ...
            if step4_status != 200 or not isinstance(step4_data, dict) or not step4_data.get("documents"):
                raise Exception(f"Không thể lấy URL tải lên: {step4_data}")

            documents = step4_data["documents"]
            if len(documents) != len(assets):
                raise Exception("Số lượng tác vụ tải lên trả về không khớp với số lượng tệp")
            ...
            logger.info(f"✅ Hoàn tất gửi tài liệu: {getattr(step6_data, 'get', lambda k, d=None: d)('currentStep')}")

            # Lấy trạng thái cuối cùng (bao gồm rewardCode)
...
def main():
    ...
    print("=" * 60)
    print("Công cụ xác thực danh tính giáo viên SheerID (Phiên bản Python)")
    print("=" * 60)
    print()
    ...
    if len(sys.argv) > 1:
        ...
        url = input("Vui lòng nhập liên kết cổng xác thực SheerID (bao gồm externalUserId): ").strip()
    ...
    print(f"👉 Sử dụng liên kết: {verifier.install_page_url}")
    if verifier.verification_id:
        print(f"ID xác thực đã giải mã: {verifier.verification_id}")
